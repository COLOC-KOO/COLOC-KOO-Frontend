...
export function HomeHero({ mode, onModeChange, searchTerm, onSearchTermChange, suggestions, onSearch }: HomeHeroProps) {
  ...
  const highlightWord = 'colocation'
  const highlightIndex = heroTitle.toLowerCase().indexOf(highlightWord)
  const titleBefore = highlightIndex >= 0 ? heroTitle.slice(0, highlightIndex) : heroTitle
  const titleHighlight = highlightIndex >= 0 ? heroTitle.slice(highlightIndex, highlightIndex + highlightWord.length) : ''
  const titleAfter = highlightIndex >= 0 ? heroTitle.slice(highlightIndex + highlightWord.length) : ''
  ...
  return <section className="relative min-h-[300px] overflow-visible bg-gray-900">
    ...
    <div
      className={liteMode ? 'absolute inset-0' : 'absolute inset-0 bg-gradient-to-r from-black/90 via-black/60 to-black/10'}
      style={liteMode ? { background: 'linear-gradient(to right, rgba(0,0,0,.9), rgba(0,0,0,.6) 50%, #d9dfd1)' } : undefined}
    />
    <div className="relative z-10 flex flex-col items-center px-4 py-8 pb-7 text-center">
      <h1 className="bebas mb-1.5 px-4 text-[40px] leading-tight tracking-wide text-white">{titleBefore}<span className="text-brand-green">{titleHighlight}</span>{titleAfter}</h1>
      ...
      <div className="mt-5 flex gap-4 rounded-xl bg-white/10 px-5 py-2.5">{[['120', 'annonces actives'], ['12', 'villes couvertes'], ['850', 'colocataires']].map(([number, label], index) => <div key={label} className={`text-center ${index > 0 ? 'border-l border-white/15 pl-4' : ''}`}><div className="bebas text-2xl tracking-wide text-white">{number}</div><div className="mt-0.5 text-[10px] text-white/50">{label}</div></div>)}</div>
    </div>
  </section>
}
