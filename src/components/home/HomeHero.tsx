...
export function HomeHero({ mode, onModeChange, searchTerm, onSearchTermChange, suggestions, onSearch }: HomeHeroProps) {
  ...
  return <section className="relative min-h-[300px] overflow-visible bg-gray-900">
    ...
    <div className="relative z-10 flex flex-col items-center px-4 py-8 pb-7 text-center">
      ...
      <div className="relative w-full max-w-[680px] sm:w-4/5"><form onSubmit={(event) => { event.preventDefault(); submitSearch() }}><div className="flex items-center gap-2 rounded-xl bg-white py-1.5 pl-3.5 pr-1.5 shadow-lg"><MapPin className="h-5 w-5 shrink-0 text-brand-cyan" /><button type="button" onClick={() => setShowSuggestions(true)} className="min-w-0 flex-1 bg-transparent text-left text-sm text-foreground outline-none">{hasSelectedCity ? searchTerm : t('home:hero.placeholder')}</button><button type="submit" className="shrink-0 rounded-lg bg-brand-green px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-brand-green-dark">{t('common:common.search')}</button></div></form>
        {showSuggestions && visibleSuggestions.length > 0 && <div className="absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-xl border border-gray-100 bg-white text-left shadow-xl">{visibleSuggestions.map((city) => <button key={city} type="button" onMouseDown={() => chooseCity(city)} className="flex w-full items-center gap-2 border-b border-gray-100 px-3.5 py-2.5 text-left text-sm text-foreground transition-colors last:border-none hover:bg-gray-50"><MapPin className="h-4 w-4 shrink-0 text-brand-cyan" /><span className="flex-1">{city}</span></button>)}</div>}
      </div>
