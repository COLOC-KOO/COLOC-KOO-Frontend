...
export default function Compte() {
  // Chargement du namespace 'tableauCompte'
  ...
  const getInitialTab = () => {
    ...
    if (requestedTab === 'paiements' || requestedTab === 'messages') {
      return 'paiements'
    }
    ...
    if (requestedTab === 'alertes') {
      return 'alertes'
    }

    if (requestedTab === 'favoris') {
      return 'favoris'
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      'colockoo:counters-refresh',
      refreshCounters
    )

    window.addEventListener(
      'colockoo:favori-removed',
      refreshCounters
    )
  ...
  return (
    <SiteLayout>
          ...
          <div className="ml-auto hidden md:flex items-center gap-2">
            ...
            {user && (
              <Button
                variant="outline"
                onClick={() => logout()}
                className="gap-2 bg-white/10 text-red-300 border-red-300/30 hover:bg-red-500 hover:text-white"
              >
                {t('logout')}
              </Button>
            )}
      ...
      <div className="w-full px-4 sm:px-6 py-8 max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-[220px_1fr] gap-6 items-start">
            ...
            {tabs.map((tItem) => {
              ...
              return (
                ...
                >
                  ...
                  {count > 0 && (
                    <span className="bg-brand-cyan text-white text-[10px] rounded-full px-1.5">
                      {count}
                    </span>
                  )}
          ...
          >
            ...
            {/* SEULE MODIFICATION : redirection après suppression */}
            {tab === 'secu' && (
              <TabCompteDonnees
      ...
      <nav
        className="md:hidden fixed bottom-0 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)]"
        style={{
          paddingBottom:
