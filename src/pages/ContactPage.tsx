...
export default function ContactPage() {
  ...
  const CONTACTS = [
    { icon: 'ti-map-pin', label: 'Adresse', val: 'Immeuble ARO Ampefiloha, Étg.1 Esc.B, Porte B104\n101 Antananarivo, Madagascar', color: 'text-sc-cy' },
    { icon: 'ti-mail', label: 'Email', val: 'contact@sarintanycoloc.mg', color: 'text-sc-y2' },
    { icon: 'ti-phone', label: 'Téléphone', val: '+261 34 00 000 00', color: 'text-sc-cy' },
    { icon: 'ti-clock', label: 'Horaires', val: 'Lun — Ven · 8h – 17h\nSam · 9h – 12h', color: 'text-sc-y1' },
  ];

  const FAQ = [
    { q: 'Comment déposer une annonce ?', r: 'Créez un compte gratuit, puis cliquez sur "Déposer une annonce". Notre équipe vérifie et publie votre annonce sous 24h.' },
    { q: 'Est-ce que le service est vraiment gratuit ?', r: 'Oui, Sarintany\'COLOC est 100% gratuit pour les colocataires et les propriétaires. Nous sommes financés par nos partenaires.' },
    { q: 'Comment fonctionne la vérification des annonces ?', r: 'Notre équipe de modérateurs vérifie chaque annonce avant publication : photos, prix, informations du propriétaire.' },
    { q: 'Comment signaler une annonce suspecte ?', r: 'Utilisez le bouton "Signaler" sur chaque annonce. Notre équipe traite les signalements sous 2h.' },
  ];
    ...
    <div className="min-h-screen bg-sc-bg">
      ...
      <div className="bg-sc-dark py-10 px-5 text-center">
        ...
        <p className="text-sm text-white/60 max-w-md mx-auto">
          Notre équipe est disponible pour répondre à toutes vos questions sur Sarintany'COLOC.
        </p>
      </div>
          ...
          <div>
            {sent ? (
              <div className="bg-white rounded-2xl p-8 text-center border border-sc-bd">
                ...
                <h3 className="font-bebas text-2xl text-sc-dark mb-2">Message envoyé !</h3>
                <p className="text-sm text-sc-gr1 mb-4">Nous vous répondrons dans les 24 heures.</p>
                <button
              ...
              <div className="bg-white rounded-2xl p-6 border border-sc-bd">
                ...
                <p className="text-xs text-sc-gr2 mb-4">Réponse garantie sous 24h</p>
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-sc-cy text-white border-none rounded-xl py-3 text-sm font-bold cursor-pointer hover:bg-sc-cy-d transition-colors flex items-center justify-center gap-2"
                  >
