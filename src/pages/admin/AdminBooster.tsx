...
// ============================================================================
//  COMPOSANT : MODALE D'AJOUT/MODIFICATION
// ============================================================================
...
const BoosterModal = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!nom.trim()) {
      alert('Veuillez saisir un nom pour l\'offre de boost')
      return
    }
    if (prix <= 0) {
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold">
              {initialData ? 'Modifier l\'offre de boost' : 'Ajouter une offre de boost'}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 outline-none focus:border-brand-cyan/50 text-white"
              placeholder="Description de l'offre de boost..."
            />
          </div>
          <div>
...
export default function AdminBooster() {
  const [boosters, setBoosters] = useState<ServiceBooster[]>([])
  ...
  const loadBoosters = async () => {
    ...
    try {
      ...
      // Filtrer uniquement les services avec clé commençant par "boost_"
      const filtered = (services || [])
        .filter((s: any) => s.cle_service && s.cle_service.startsWith('boost_'))
        .map((s: any) => ({
      ...
      setBoosters(filtered)
      setSuccessMessage('Offres de boost chargées')
      setTimeout(() => setSuccessMessage(null), 3000)
    } catch (err) {
      console.error('❌ Erreur:', err)
      setError(err instanceof Error ? err.message : 'Impossible de charger les offres de boost')
    } finally {
      setLoading(false)
    }
  }
  ...
  const filteredBoosters = boosters.filter((b) => {
    const matchSearch = b.nom.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const stats = {
    ...
    actif: boosters.filter(b => b.est_actif === 1).length,
    inactif: boosters.filter(b => b.est_actif === 0).length,
    prixMoyen: boosters.length > 0 
      ? Math.round(boosters.reduce((sum, b) => sum + b.prix, 0) / boosters.length)
      : 0,
  }
  ...
  const handleAddBooster = async (data: BoosterFormData) => {
    setLoading(true)
    setError(null)
    try {
      const cleService = `boost_${Date.now()}`
      ...
      await loadBoosters()
      setShowModal(false)
      setSuccessMessage(`Offre "${data.nom}" ajoutée avec succès`)
      setTimeout(() => setSuccessMessage(null), 3000)
    } catch (err) {
  ...
  const handleEditBooster = async (data: BoosterFormData) => {
    if (!editingBooster) return
    setLoading(true)
    setError(null)
    try {
      await api.updateServiceCkoo(String(editingBooster.id_service), {
        nom: data.nom,
        description: data.description || '',
        prix: data.prix,
        unite: data.unite || 'jour',
      ...
      setSuccessMessage(`Offre "${data.nom}" modifiée avec succès`)
      setTimeout(() => setSuccessMessage(null), 3000)
    } catch (err) {
  ...
  const handleDeleteBooster = async (id: number, nom: string) => {
    ...
    try {
      ...
      await loadBoosters()
      setSuccessMessage(`Offre "${nom}" supprimée avec succès`)
      setTimeout(() => setSuccessMessage(null), 3000)
    } catch (err) {
  ...
  const handleToggleBooster = async (id: number, currentStatus: 0 | 1, nom: string) => {
    ...
    try {
      ...
      await loadBoosters()
      setSuccessMessage(`Offre "${nom}" ${newStatus === 1 ? 'activée' : 'désactivée'}`)
      setTimeout(() => setSuccessMessage(null), 3000)
    } catch (err) {
  ...
  const handleModalSave = (data: BoosterFormData) => {
    if (editingBooster) {
      handleEditBooster(data)
    } else {
      handleAddBooster(data)
    }
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <h1 className="bebas text-3xl text-white">Boost Annonce</h1>
            </div>
        ...
        <div className="bg-[oklch(0.22_0.005_260)] border border-white/10 rounded-2xl overflow-hidden">
          ...
          <div className="p-4 border-b border-white/10">
            <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 flex-1 max-w-xs">
                ...
                <input
                  placeholder="Rechercher une offre de boost..."
          ...
          {loading && boosters.length === 0 ? (
            <div className="text-center py-16 text-white/40">
              ...
              <p className="text-sm">Chargement des offres de boost...</p>
            </div>
          ) : filteredBoosters.length === 0 ? (
            <div className="text-center py-16 text-white/40">
              <Rocket className="w-12 h-12 mx-auto mb-3 opacity-20" />
              <p className="text-sm">Aucune offre de boost trouvée</p>
              {boosters.length === 0 && (
                <button
                  onClick={openAddModal}
                    ...
                    <tr key={booster.id_service} className="hover:bg-white/5 transition">
                      ...
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            onClick={() => handleToggleBooster(booster.id_service, booster.est_actif, booster.nom)}
                          ...
                          <button
                            onClick={() => handleDeleteBooster(booster.id_service, booster.nom)}
          ...
          <div className="p-4 border-t border-white/10 flex flex-wrap items-center gap-4 text-xs text-white/40">
            ...
            <span>·</span>
            <span className="text-brand-green">{filteredBoosters.filter(b => b.est_actif === 1).length} active{filteredBoosters.filter(b => b.est_actif === 1).length > 1 ? 's' : ''}</span>
            <span>·</span>
            <span className="text-red-400">{filteredBoosters.filter(b => b.est_actif === 0).length} inactive{filteredBoosters.filter(b => b.est_actif === 0).length > 1 ? 's' : ''}</span>
        ...
        <div className="bg-[oklch(0.22_0.005_260)] border border-white/10 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-xs text-white/40">
              <p>Les offres de <b className="text-brand-cyan">Boost Annonce</b> permettent aux utilisateurs de mettre en avant leur annonce.</p>
              <p className="mt-1">
                Seules les offres avec une clé commençant par <b className="text-brand-cyan">"boost_"</b> sont affichées dans ce tableau.
                Les offres actives sont visibles par les utilisateurs pour acheter un boost.
              </p>
            </div>
          </div>
        </div>
