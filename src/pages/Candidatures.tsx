...
// import { CandidaturesViewButtons } from "../components/candidatures/CandidaturesHeader";
...
//   return (
//     <SiteLayout>
//       <div className="mx-auto min-h-[calc(100vh-120px)] max-w-2xl px-4 py-6 sm:px-6">
//         <div
//           className="sticky top-20 z-40 mb-3 w-screen border-b border-border bg-background pb-3 pt-2"
...
//           {/* Welcome message after successful contract */}
//           {officialNotification === "won" && myContracts.length > 0 && (
//             <div className="mx-auto mt-3 max-w-6xl px-4 sm:px-6">
//               <div className="rounded-2xl border border-brand-green/30 bg-brand-green-light/20 p-4">
...
//                         FÉLICITATIONS, {currentCandidateName.toUpperCase()} !
//                       </h3>
//                       <p className="text-sm text-muted-foreground">
//                         Ta candidature est retenue : tu fais partie de la colocation {logementTitre}. Bienvenue !
//                       </p>
//                     </div>
//                   </div>
//                   <div className="flex items-center gap-2 text-sm text-muted-foreground">
//                     <Calendar className="h-4 w-4 text-brand-cyan-dark" />
//                     <span>Emménagement — {moveInLabel}</span>
//                   </div>
//                 </div>
//               </div>
//             </div>
...
//         <section className="mx-auto mb-6 flex max-w-2xl items-center gap-4 rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5">
//           <div className="h-16 w-16 shrink-0 rounded-xl bg-brand-cyan-light sm:h-20 sm:w-20" />
//           <div className="min-w-0">
//             <p className="bebas truncate text-xl leading-tight text-brand-dark sm:text-2xl">
//               {logementTitre} · {logementResume}
//             </p>
//             <p className="mt-1 text-sm font-bold text-brand-dark sm:text-base">
...
//                     {wonMode === "indiv"
//                       ? `Félicitations, ${currentCandidateName} !`
      ...
      <div className="rounded-2xl border border-brand-green/30 bg-brand-green-light/20 p-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-brand-green-dark">
                FÉLICITATIONS, {candidateName.toUpperCase()} !
              </h3>
              <p className="text-sm text-muted-foreground">
                Ta candidature est retenue : tu fais partie de la colocation {logementTitre}. Bienvenue !
              </p>
...
// ===== FONCTION UTILITAIRE POUR VÉRIFIER SI UN CONTRAT EST PAYÉ =====
const isContractPaid = (contract: any): boolean => {
  // Vérifier via statut_paiement
  if (contract.statut_paiement) {
    return contract.statut_paiement.tous_payes === true;
  }
  // Vérifier via les propriétés directes
  if (contract.tous_payes !== undefined) {
    return contract.tous_payes === true;
  }
  // Vérifier via paye/total
  if (contract.paye !== undefined && contract.total !== undefined) {
    return contract.paye >= contract.total;
  }
  // Vérifier via paidCount/total (format utilisé dans l'UI)
  if (contract.paidCount !== undefined && contract.total !== undefined) {
    return contract.paidCount >= contract.total;
  }
  // Fallback : considérer que c'est payé si deja_paye est true
  return contract.deja_paye === true;
};
...
export default function Candidatures() {
  ...
  // ===== ÉTAT POUR LE BANNER DE BIENVENUE =====
  ...
  // ===== NOM DU COLOCATAIRE POUR LE BANNER =====
  // Priorité : prénom + nom de la candidature, puis prénom/nom de l'utilisateur
  const colocataireNom = useMemo(() => {
    if (currentUserCandidature?.prenom && currentUserCandidature?.nom) {
      return `${currentUserCandidature.prenom} ${currentUserCandidature.nom}`;
    }
    if (currentUserCandidature?.prenom) {
      return currentUserCandidature.prenom;
    }
    if (currentUserCandidature?.nom) {
      return currentUserCandidature.nom;
    }
    ...
    return userName;
  }, [currentUserCandidature, user, userName]);
  ...
  // ===== EFFET POUR RAFRAÎCHIR LES CONTRATS QUAND L'UTILISATEUR EST RETENU =====
  useEffect(() => {
    if (isCurrentUserRetained && annonceId) {
      refreshMyContracts();
    }
  ...
  useEffect(() => {
    ...
    // Vérifier si le contrat est signé (tous les paiements effectués)
    const allContractsPaid = myContracts.length > 0 && myContracts.every(isContractPaid);
    
    // Afficher le message si l'utilisateur est retenu et que le contrat est validé
    if (isRetainedAndContractValid && allContractsPaid && !hasShownWelcome) {
      setHasShownWelcome(true);
    }
    
    // Si l'utilisateur n'est plus retenu ou n'a plus de contrats, cacher le message
  ...
  const retainedTeamLabel =
    retainedCandidateNames.length > 0
      ...
      : activeWinner?.members.join(", ") || t('contract.parties');
  const retainedTeamTitle = activeWinner?.title || t('contract.parties');
  const wonIndividualMessage = t('notifications.congratsName', { name: colocataireNom }) + " " + t('notifications.moveIn') + " " + moveInLabel + ".";
  const wonGroupMessage = t('notifications.congratsTeam', { team: retainedTeamTitle }) + " " + t('notifications.moveIn') + " " + moveInLabel + ".";
  const lostIndividualMessage = t('notifications.notThisTime') + " " + logementTitre + ".";
  const lostGroupMessage = t('notifications.thanksApplication') + " " + logementTitre + ".";
  ...
  async function launchOfficial() {
    ...
    try {
      ...
      setOfficialLaunched(true);
      setAnnonceData((prev) => (prev ? { ...prev, statut: "terminee" } : prev));
      await refreshMyContracts();
      
      // Vérifier si tous les contrats sont payés après le lancement officiel
      const allPaid = myContracts.length > 0 && myContracts.every(isContractPaid);
      if (allPaid && isCurrentUserRetained) {
        setHasShownWelcome(true);
      }
    } catch (error: any) {
      setOfficialFeedback(
  ...
  async function finalizeContract() {
    ...
    try {
      ...
      // Rafraîchir les contrats immédiatement
      await refreshMyContracts();
      
      // Vérifier si le contrat est payé
      if (contracts.length > 0) {
        const allPaid = contracts.every(isContractPaid);
        if (allPaid && isCurrentUserRetained) {
          setHasShownWelcome(true);
        }
      }
        ...
        {!authLoading && !user ? (
          ...
          <div className="mt-8 space-y-6">
            ...
            {activeView === "track" &&
              ...
              myContracts.length > 0 &&
              (() => {
                const allPaid = myContracts.every(isContractPaid);
                const isFinalized = annonceData?.statut === "terminee";
                return (
                  <div className="mb-6 rounded-3xl border border-brand-cyan/30 bg-white p-6 shadow-sm">
                    <div className="bebas text-2xl text-brand-cyan-dark">
                      ...
                      .map((c) => (
                        ...
                        >
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {t('cand.yourShare', { amount: fmtAr(c.ma_part) })}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
