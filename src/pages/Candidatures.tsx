...
export default function Candidatures() {
  ...
  // ===== Etat de validation d'equipe : derive UNIQUEMENT du statut persiste en DB =====
  const validatedTeam =
    teams.find((team) => team.statut === "selected" || team.statut === "complete") ||
    null;
  const activeWinner = validatedTeam || winnerTeam;
  const isCourseFinished = Boolean(validatedTeam);
  ...
  // ===== EFFETS =====
  useEffect(() => {
    if (winnerTeam && !validatedTeam && !waitDeadline) {
      setWaitDeadline(Date.now() + 3 * 864e5);
    }
  }, [winnerTeam, validatedTeam, waitDeadline]);

  useEffect(() => {
    if (!waitDeadline || validatedTeam) return;
  ...
  // ===== VALIDER UNE ÉQUIPE (persiste en DB via PUT /equipes/:id) =====
  async function validateTeam(id: string) {
    try {
      await api.updateEquipe(id, { statut: 'selected' });
      await loadEquipes(); // recharge depuis la DB -> persiste au refresh
      setCelebrateOpen(true);
    } catch (error: any) {
      setCandidateActionFeedback(error?.message || t('common.loading'));
    }
  }
