...
export default function BackofficePage() {
  ...
  if (auth !== 'admin' && auth !== 'moderator') {
    return (
      <div className="min-h-screen bg-bo-bg flex items-center justify-center p-4">
        <div className="bg-bo-panel border border-bo-line rounded-2xl p-6 sm:p-8 max-w-md w-full text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-2 justify-center">
            <button 
              ...
              className="px-4 py-2.5 bg-sc-cy text-white border-none rounded-xl text-sm font-bold cursor-pointer hover:bg-sc-cy-d transition-colors"
            >
              Démo Modérateur
            </button>
            <button 
              onClick={() => login('admin')} 
              className="px-4 py-2.5 bg-sc-y2 text-white border-none rounded-xl text-sm font-bold cursor-pointer hover:opacity-90 transition-opacity"
            >
              Démo Admin
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-bo-bg text-bo-txt font-sans">
      {/* Demo bar */}
      <div className="sticky top-0 z-50 bg-black border-b border-bo-line px-3 sm:px-4 py-2 flex items-center gap-2 sm:gap-3 flex-wrap text-xs">
        <span className="text-sc-y1 font-bold tracking-wider uppercase text-[10px] sm:text-xs">Back-office</span>
        <div className="flex gap-1.5">
          {(['moderator', 'admin'] as const).map(r => (
            <button
              key={r}
              onClick={() => { setDemoRole(r); login(r); }}
              className={`px-2 sm:px-2.5 py-1.5 rounded-lg border text-[10px] sm:text-xs font-bold cursor-pointer transition-colors ${
                demoRole === r
        ...
        <div className="ml-auto text-bo-muted hidden sm:block">
          Connecté en tant que <strong className="text-bo-txt">{user?.name}</strong>
        </div>
        <button 
          onClick={() => navigate('/')} 
          className="flex items-center gap-1 text-bo-muted hover:text-bo-txt transition-colors ml-auto sm:ml-0"
        >
        ...
        <button
          ...
          className="md:hidden text-bo-muted hover:text-bo-txt transition-colors"
        >
          <i className={`ti ${isMobileMenuOpen ? 'ti-x' : 'ti-menu-2'} text-lg`} />
        </button>
      </div>

      <div className="flex min-h-[calc(100vh-39px)] relative">
        {/* Sidebar - Desktop */}
        <aside className={`hidden md:block w-60 bg-bo-panel border-r border-bo-line p-3 sticky top-[39px] h-[calc(100vh-39px)] overflow-y-auto flex-shrink-0`}>
        ...
        {/* Sidebar - Mobile */}
        <aside className={`fixed top-[39px] left-0 w-72 max-w-[85vw] h-[calc(100vh-39px)] bg-bo-panel border-r border-bo-line p-3 overflow-y-auto z-50 transition-transform duration-300 ease-in-out md:hidden ${
        ...
        {/* Main content */}
        <main className="flex-1 p-3 sm:p-4 md:p-6 max-w-[1180px] overflow-x-hidden">
          ...
          {(section === 'signalements' || section === 'partenaires_bo' || section === 'parametres') && (
            <div className="text-center py-12 sm:py-16 text-bo-muted">
              <i className="ti ti-tools text-4xl sm:text-5xl block mb-4" />
              <h3 className="font-bebas text-xl sm:text-2xl text-bo-txt mb-2">Section en développement</h3>
              <p className="text-sm sm:text-base">Cette section sera disponible prochainement.</p>
            </div>
          )}
        </main>
      </div>
