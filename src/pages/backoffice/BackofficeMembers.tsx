  ...
  return (
    <div>
      <div className="flex items-end gap-3 flex-wrap mb-5">
        ...
        <p className="text-bo-muted text-sm">{MEMBRES.length} membres enregistrés</p>
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        {[
          { val: '847', label: 'Total membres', color: 'text-sc-cy' },
          { val: '12', label: 'Nouveaux cette semaine', color: 'text-[#99CC33]' },
          { val: '3', label: 'Suspendus', color: 'text-[#E2B53A]' },
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-bo-line">
              {['Membre', 'Rôle', 'Statut', 'Activité', 'Inscription', 'Actions'].map(h => (
                <th key={h} className="text-left px-4 py-3 text-[11px] font-bold text-bo-muted uppercase tracking-wider">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {filtered.map(m => (
              <tr key={m.id} className="border-b border-bo-line last:border-none hover:bg-bo-panel2 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${ROLE_STYLE[m.role] || ''}`}>
                    {m.role}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                    m.statut === 'actif' ? 'bg-[rgba(153,204,51,.12)] text-[#c0e07a]' : 'bg-[rgba(224,96,78,.12)] text-[#f0a094]'
                  }`}>
                    {m.statut}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs text-bo-muted">
                  {m.annonces > 0 ? `${m.annonces} annonce${m.annonces > 1 ? 's' : ''}` : ''}
                  {m.candidatures > 0 ? `${m.candidatures} candidature${m.candidatures > 1 ? 's' : ''}` : ''}
                  {m.annonces === 0 && m.candidatures === 0 ? 'Inactif' : ''}
                </td>
                <td className="px-4 py-3 text-xs text-bo-muted">{m.inscription}</td>
                <td className="px-4 py-3">
                  <div className="flex gap-1.5">
                    <button className="px-2.5 py-1.5 bg-bo-panel2 border border-bo-line rounded-lg text-[11px] text-bo-txt cursor-pointer hover:border-bo-muted transition-colors">
                      Voir
                    </button>
                    {m.statut === 'actif' ? (
                      <button className="px-2.5 py-1.5 bg-transparent border border-[rgba(226,181,58,.4)] text-[#e8c97a] rounded-lg text-[11px] cursor-pointer hover:bg-[rgba(226,181,58,.08)] transition-colors">
                        Suspendre
                      </button>
                    ) : (
                      <button className="px-2.5 py-1.5 bg-transparent border border-[rgba(153,204,51,.4)] text-[#c0e07a] rounded-lg text-[11px] cursor-pointer hover:bg-[rgba(153,204,51,.08)] transition-colors">
                        Réactiver
                      </button>
                    )}
                  </div>
