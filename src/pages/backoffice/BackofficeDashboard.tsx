...
const RECENT_ACTIONS = [
  { id: 'a1', type: 'validation', texte: 'Annonce #142 validee - Villa Iavoloha', user: 'Moderateur', temps: 'il y a 3 min', color: 'bg-[rgba(153,204,51,.15)] text-[#c0e07a]' },
  { id: 'a2', type: 'refus', texte: 'Annonce #141 refusee - Photos non conformes', user: 'Moderateur', temps: 'il y a 12 min', color: 'bg-[rgba(224,96,78,.15)] text-[#f0a094]' },
  { id: 'a3', type: 'signal', texte: 'Signalement traite - Membre @rakoto_a', user: 'Moderateur', temps: 'il y a 25 min', color: 'bg-[rgba(205,108,168,.15)] text-[#e3a8cd]' },
  { id: 'a4', type: 'validation', texte: 'Annonce #140 validee - Appart Analakely', user: 'Moderateur', temps: 'il y a 1h', color: 'bg-[rgba(153,204,51,.15)] text-[#c0e07a]' },
  { id: 'a5', type: 'correction', texte: 'Annonce #139 renvoyee - Correction demandee', user: 'Moderateur', temps: 'il y a 1h 30', color: 'bg-[rgba(226,181,58,.15)] text-[#e8c97a]' },
];

const FALLBACK_KPIS = [
  { val: '12', label: 'En file d\'attente', color: 'text-sc-cy', icon: 'ti-stack-2' },
  { val: '8', label: 'Validees aujourd\'hui', color: 'text-[#99CC33]', icon: 'ti-circle-check' },
...
export default function BackofficeDashboard() {
  ...
  useEffect(() => {
    if (!token) return;
    api.request('/backoffice/dashboard', { token })
      .then(setDashboard)
      .catch(() => null);
  }, [token]);
  ...
  const kpis = dashboard
    ? [
        { val: String(dashboard.annoncesFile ?? 0), label: 'En file d\'attente', color: 'text-sc-cy', icon: 'ti-stack-2' },
        { val: String(dashboard.validationsAujourdhui ?? 0), label: 'Validees aujourd\'hui', color: 'text-[#99CC33]', icon: 'ti-circle-check' },
        { val: String(dashboard.signalements ?? 0), label: 'Signalements actifs', color: 'text-[#E2B53A]', icon: 'ti-flag' },
        { val: String(dashboard.membresActifs ?? 0), label: 'Membres actifs', color: 'text-[#CD6CA8]', icon: 'ti-users' },
        { val: String(dashboard.annoncesMois ?? 0), label: 'Annonces ce mois', color: 'text-[#99CC33]', icon: 'ti-home' },
        { val: `${dashboard.tauxValidation ?? 0}%`, label: 'Taux validation', color: 'text-sc-cy', icon: 'ti-chart-bar' },
      ]
    : FALLBACK_KPIS;
  ...
  return (
    <div>
      <div className="flex items-end gap-3 flex-wrap mb-5">
        ...
        <p className="text-bo-muted text-sm">Vue globale de la plateforme</p>
      ...
      <div className="bg-bo-panel border border-bo-line rounded-xl p-4 mb-4">
        ...
        <div className="h-3 bg-bo-panel2 rounded-full overflow-hidden border border-bo-line mb-2">
          <div className="h-full rounded-full transition-all" style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #CCCC33, #99CC33)' }} />
        </div>
        <p className="text-xs text-bo-muted">
          {objectif - fait} annonces restantes pour atteindre l'objectif fixe par le Super Admin
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-bo-panel border border-bo-line rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            <h2 className="font-bebas text-xl text-bo-txt flex-1">Activite recente</h2>
            <button className="text-xs text-sc-cy hover:underline cursor-pointer bg-none border-none font-sans">Tout voir</button>
          </div>
          <div className="space-y-2">
            {RECENT_ACTIONS.map(a => (
              <div key={a.id} className="flex items-start gap-2.5 py-2 border-b border-bo-line last:border-none">
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${a.color}`}>{a.type}</span>
        ...
        <div className="bg-bo-panel border border-bo-line rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs bg-[rgba(224,96,78,.15)] text-[#f0a094] border border-[rgba(224,96,78,.35)] px-2 py-0.5 rounded-full font-bold">
              {dashboard?.annoncesFile ?? 12} en attente
            </span>
          </div>
          <div className="space-y-2">
            {[
              ...
              <div key={item.id} className="flex items-center gap-2 py-2 border-b border-bo-line last:border-none">
                ...
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${item.type === 'Proprio' ? 'bg-[rgba(70,189,214,.12)] text-[#8fd9e8]' : 'bg-[rgba(153,204,51,.12)] text-[#c0e07a]'}`}>{item.type}</span>
                {item.priorite === 'haute' && <span className="text-[10px] text-[#E2B53A]"><i className="ti ti-alert-triangle text-xs" /> Priorite</span>}
                <button className="text-xs text-sc-cy font-bold bg-none border-none cursor-pointer hover:underline">Traiter →</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-bo-panel border border-bo-line rounded-xl p-4 mt-4">
          ...
          {[
            ...
            <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
              <div className="w-full flex flex-col items-center gap-0.5">
                <div className={`w-full rounded-t-sm ${d.current ? 'bg-sc-cy' : 'bg-[rgba(153,204,51,.4)]'}`} style={{ height: `${(d.ok / 35) * 80}px` }} title={`${d.ok} valides`} />
                {d.ref > 0 && <div className="w-full bg-[rgba(224,96,78,.4)]" style={{ height: `${(d.ref / 35) * 80}px` }} title={`${d.ref} refusees`} />}
              </div>
        ...
        <div className="flex gap-4 mt-2">
          <span className="flex items-center gap-1.5 text-[10px] text-bo-muted"><span className="w-3 h-2 rounded-sm bg-[rgba(153,204,51,.6)] inline-block" /> Validees</span>
          <span className="flex items-center gap-1.5 text-[10px] text-bo-muted"><span className="w-3 h-2 rounded-sm bg-[rgba(224,96,78,.5)] inline-block" /> Refusees</span>
        </div>
      </div>
    </div>
  );
